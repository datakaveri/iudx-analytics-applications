...
trim_now = datetime.now(IST)-timedelta(minutes=15)
...
curr_time = int(time.mktime(time.strptime(curr_time, "'%Y-%m-%d %H:%M:%S'")))
print(curr_time)
...
eta = eta.select('primary_key', 'trip_id', 'id', 'route_id', 'trip_direction', 'actual_trip_start_time', 'vehicle_label', 'license_plate', 'last_stop_id', 'speed', 'observationDateTime', 'trip_delay', 'location_type', 'latitude', 'longitude')
...
w = Window.partitionBy('trip_id')
eta_max_min = eta.withColumn('min_actual_trip_start_time', f.min('actual_trip_start_time').over(w))\
    .withColumn('max_observationDateTime', f.max('observationDateTime').over(w))\
    .withColumn('min_observationDateTime', f.min('observationDateTime').over(w))\
    .withColumn('max_latitude', f.max('latitude').over(w))\
    .withColumn('max_longitude', f.min('longitude').over(w))\
    .where((f.col('min_actual_trip_start_time') == f.col('actual_trip_start_time')) | (f.col('max_longitude') == f.col('longitude')) | (f.col('max_latitude') == f.col('latitude')) | (f.col('observationDateTime') == f.col('max_observationDateTime')) | (f.col('observationDateTime') == f.col('min_observationDateTime'))).select("primary_key","trip_id","min_actual_trip_start_time","min_observationDateTime","max_latitude","max_longitude","max_observationDateTime","license_plate").dropDuplicates(["trip_id","min_actual_trip_start_time","max_latitude","max_longitude","max_observationDateTime", "max_observationDateTime", "license_plate"]).sort("trip_id")
...
lateness_threshold = 3600
lateness_threshold_ratio = 0.6

# merged_df = merged_df.withColumn("status", f.when((f.col("percent")>=dist_threshold_ratio) & (f.abs(f.col("max_observationDateTime")-f.col("end_time"))<lateness_threshold) & (f.abs(f.col("min_observationDateTime")-f.col("start_time"))<lateness_threshold) , "COMPLETED").when((f.col("percent")>=dist_threshold_ratio) & (f.abs(f.col("max_observationDateTime")-f.col("end_time"))>=lateness_threshold) | (f.abs(f.col("min_observationDateTime")-f.col("start_time"))>=lateness_threshold) , "LATE").when((f.col("percent")<dist_threshold_ratio) & (f.abs((f.col("max_observationDateTime")-f.col("min_observationDateTime")) /(f.col("end_time")-f.col("start_time")))>lateness_threshold_ratio) , "INCOMPLETE").when((f.col("percent")<dist_threshold_ratio) & (f.abs((f.col("max_observationDateTime")-f.col("min_observationDateTime")) /(f.col("end_time")-f.col("start_time")))<=lateness_threshold_ratio) , "LIVE").otherwise("MISSED"))
merged_df = merged_df.withColumn("status", f.when((f.col("percent")<dist_threshold_ratio) & (f.abs((f.col("max_observationDateTime")-f.col("min_observationDateTime")) /(f.col("end_time")-f.col("start_time")))<=lateness_threshold_ratio) , "LIVE").when((f.col("percent")<dist_threshold_ratio) & (f.abs((f.col("max_observationDateTime")-f.col("min_observationDateTime")) /(f.col("end_time")-f.col("start_time")))>lateness_threshold_ratio) , "INCOMPLETE").when((f.col("percent")>=dist_threshold_ratio) & (f.abs(f.col("max_observationDateTime")-f.col("end_time"))<lateness_threshold) & (f.abs(f.col("min_observationDateTime")-f.col("start_time"))<lateness_threshold) , "COMPLETED").when((f.col("percent")>=dist_threshold_ratio) & (f.abs(f.col("max_observationDateTime")-f.col("end_time"))>=lateness_threshold) | (f.abs(f.col("min_observationDateTime")-f.col("start_time"))>=lateness_threshold) , "LATE").otherwise("MISSED"))
...
merged_df = merged_df.withColumn("status",f.when((f.col("status").isNull()) & (curr_time<f.col("stop_start_time")),"NOT STARTED")\
            .when((f.col("status").isNull()) & (curr_time>=f.col("stop_start_time")) & (curr_time<=f.col("stop_end_time")),"NOT STARTED")\
            .when((f.col("status").isNull()) & (curr_time>f.col("stop_end_time")),"MISSED")\
            .otherwise(f.lit(f.col("status"))))


# merged_df = merged_df.withColumn("status",f.when((f.col("status").isNull()) & (f.col("stop_start_time")<curr_time),"MISSED").when((f.col("status").isNull()) & (f.col("stop_start_time")>=curr_time),"NOT STARTED").otherwise(f.lit(f.col("status"))))

"""# Concatenate Timestamp Columns to get Last Stop Arrival Time """
merged_df = merged_df.select(f.col("trip_id"),f.col("status"),f.col("license_plate"),f.col("total_sch_time").alias("trip_time"),f.concat_ws('',f.col("max_observationDateTime"),f.col("stop_end_time")).alias("observationDateTime"),f.col("max_latitude").alias("latitude"),f.col("max_longitude").alias("longitude"))
...
merged_df = merged_df.withColumn("observationDateTime", f.col("observationDateTime").cast('long'))
...
merged_df1 = merged_df[merged_df["observationDateTime"]>=trim_epoch*1000000]
